...
def matchup(sport: str, home: str, away: str, asof: str,
    ...
    def rows(off_team, off_split, def_team, def_split):
        out = []
        for label, col, dcol, _dir in specs["pairs"]:
            o = off_split.get(col, {})
            o_rank = ranks.get(col, {}).get(off_team)
            row = {"stat": label, "off_l5": o.get("l5"),
                   "off_season": o.get("season"), "off_rank": o_rank,
                   "def_l5": None, "def_season": None, "def_rank": None, "adv": 0}
            if dcol:
                d = def_split.get(dcol, {})
                d_rank = ranks.get(dcol, {}).get(def_team)
                row.update({"def_l5": d.get("l5"), "def_season": d.get("season"),
                            "def_rank": d_rank, "adv": _advantage(o_rank, d_rank)})
            out.append(row)
        return out

    return {
        "home": home, "away": away,
        "away_off_vs_home_def": rows(away_k, as_, home_k, hs),
        "home_off_vs_away_def": rows(home_k, hs, away_k, as_),
